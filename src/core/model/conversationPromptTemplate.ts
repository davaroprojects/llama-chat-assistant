...
export const DEFAULT_GLOBAL_REACT_TEMPLATE: ConversationPromptTemplate = {
    systemPrompt: [
        ...
        '- `llamachat_agent_search(query_text: string)`: Searches the vector database for relevant code structures, files, functions, or architectural implementations matching the text.',
        '',
    ...
    userPrompt: [
        ...
        'Instruction: Use the `llamachat_agent_search` tool to discover files and functions across the project. Do not guess; find the source code.',
...
export const DEFAULT_DEEP_REACT_TEMPLATE: ConversationPromptTemplate = {
    systemPrompt: [
        ...
        '',
        'You have access to a single tool:',
        '- `llamachat_agent_search(query_text: string)`: Searches the project repository for missing functions, classes, imports, or type definitions that are called but not defined in the target files.',
        '',
    ...
    userPrompt: [
        ...
        'Instruction: Analyze the target files first. Use `llamachat_agent_search` only to resolve external definitions or code references that impact this specific scope.',
        'First response requirement: emit Thought and Action only. Do not emit Final Answer before the first tool call.',
        '</execution_mode>',
        '',
        '<target_files>',
