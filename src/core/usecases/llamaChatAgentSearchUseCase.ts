...
export class LlamaChatAgentSearchUseCase {
    ...
    async execute(
        ...
        let matches = await this.ragGateway.queryByMode(
            normalizedQuery,
            config,
        ...
        if (matches.length === 0) {
            this.logger.debug('rag', 'Semantic search returned no matches; falling back to lexical search', {
                queryText: normalizedQuery
            });
            matches = await this.ragGateway.queryByMode(
                normalizedQuery,
                config,
                Math.min(config.maxQueryResults, 5),
                'lexical',
